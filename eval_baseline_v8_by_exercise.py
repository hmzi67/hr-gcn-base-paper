"""
Per-exercise evaluation for GCADA rehab checkpoints.

Loads a fine-tuned rehab checkpoint and evaluates ROM MAE breakdown by exercise
(E1-E10), matching the format of TABLE I in the thesis.

Output: CSV + table with per-exercise MAE scores.
...
Example:
    python eval_baseline_v8_by_exercise.py \
        ...
        --save_csv baseline_v8_results.csv
"""
...
# UI-PRMD exercises: 10 exercises, 10 subjects
# Exercise IDs: 0-9 (mapped to E1-E10)
EXERCISE_NAMES = [f'E{i+1}' for i in range(10)]


def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate a rehab checkpoint by exercise')
    ...
    parser.add_argument('--save_csv', default='', type=str,
                        help='Optional CSV output path for per-exercise results')
    return parser.parse_args()


class UIRPMDDatasetWithMeta(TensorDataset):
    """Load UI-PRMD with exercise metadata."""
...
class _AngleHead(nn.Module):
    """ClinicalAngleHead with configurable output dim."""
...
def detect_head_dims(angle_state):
    """Recover (hidden, n_joints) from checkpoint's angle_head state dict."""
...
def main():
    ...
    with torch.no_grad():
        for inputs_2d, targets_3d, gt_angles, exercise_ids in test_loader:
    ...
    worst_idx = int(np.argmax(rom_mae_overall))
    print(f'\nPer-Joint Mean ROM MAE:')
    print('-' * 40)
    print(f'{"Joint":<20} | {"MAE (deg)":>10}')
    print('-' * 40)
    for i, (name, mae_v) in enumerate(zip(joint_names, rom_mae_overall)):
        marker = ' *' if i == worst_idx else ''
        print(f'{name:<20} | {mae_v:>10.4f}{marker}')
    print('-' * 40)
    ...
    if args.save_csv:
        with open(args.save_csv, 'w', newline='') as f:
            ...
            for name, mae_v in zip(joint_names, rom_mae_overall):
